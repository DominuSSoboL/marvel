	...
	$.DropDown.prototype = {
		...
		_transformSelect : function() {
			...
			this.$el.children( 'option' ).each( function() {
				...
				val !== -1 ?
					classes !== undefined ?
						optshtml += '<li data-value="' + val + '" class="' + classes + '"><span class="' + classes + '">' + label + '</span></li>' :
		...
		_initEvents : function() {
			...
			this.opts.on( 'click.dropdown', function() {
				if( self.opened ) {
					...
					// Portfolio filter for mobile add my plagin options
					if(opt.attr('class') == 'all') {
				      var $filteredData = $data.find('.all');
				    } else {
				      var $filteredData = $data.find('.all[data-type=' + opt.attr('class') + ']');
				    }
